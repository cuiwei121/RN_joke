...
var GiftedListView = React.createClass({
  ...
  _onRefresh(options = {firstLoad:true}) {
    console.log('+++++++++下拉刷新++++++++++++++++_onRefresh'+options);
    if (this.isMounted()) {
      this.setState({
        isRefreshing: true,
      });
  ...
  _onPaginate(options = {firstLoad:false}) {
    console.log('+++++加载更多++++++++++++++++++++_onPaginate');
    if(this.state.paginationStatus==='allLoaded'){
      ...
      this.props.onFetch(this._getPage() + 1, this._postPaginate, options);
    }
  },
  ...
  render() {
    return (
      <ListView
        ...
        isRefreshing={this.props.isRefreshing === true ? this._onRefresh():null}
